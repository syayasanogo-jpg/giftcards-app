...
export default function Paiement({ amount, customer, onSuccess, onCancel, label="Payer (test)" }) {
  const [pubKey, setPubKey] = useState("");
  const [scriptReady, setScriptReady] = useState(false);
  const [error, setError] = useState("");

  // 1) Récup clé publique avec 2 fallbacks
  useEffect(() => {
    let alive = true;
    (async () => {
      ...
      try {
        let data;
        try { data = await tryFetch("/api/public-key"); }
        catch { data = await tryFetch("/.netlify/functions/public-key"); }
        if (alive && data?.publicKey) setPubKey(data.publicKey);
      } catch (e) {
  ...
  const handlePay = () => {
    ...
    FlutterwaveCheckout({
      ...
      customizations: { title: "Gift Cards", description: "Achat de cartes (démo)" },
      callback: (res) => (res?.status === "successful" ? onSuccess?.(res) : onCancel?.(res)),
      onclose: () => onCancel?.(),
    });
  };
  ...
  return (
    <div className="space-y-2">
      <button
        className={`w-full px-4 py-3 rounded-xl text-white ${isReady ? "bg-black hover:bg-neutral-800" : "bg-gray-300 cursor-not-allowed"}`}
        disabled={!isReady}
        onClick={handlePay}
      >
